...
private:
	...
	/**
	  * @brief Metodo auxiliar de copia.
	  * @param quienEsQuien QuienEsQuien a copiar.
	  */
	void copiar(const QuienEsQuien &quienEsQuien);

public:
	/**
	  * @brief Constructor b�sico de la clase
	  */
	QuienEsQuien() = default;
	/**
	...
	*/
	...
	int calcular_mejor_atributo(vector<int> &atrib_usados, set<int> personajes_levantados);

	void crear_arbol(const bintree<Pregunta>::node &nodoP, vector<int> atrib_usados, set<int> personajes_levantados);

	set<int> tumba_personajes(set<int> personajes_levantados, int indice, bool rama);

	Pregunta crear_pregunta(int indice, set<int> personajes_levantados);

	pair<int, int> calcular_valores_atributo(int atributo, set<int> personajes_levantados) const;

	bintree<Pregunta> eliminar_nodos_redundantes(const bintree<Pregunta>::node &nodo);

	string responder();

	double calcular_entropia(pair<int, int> valores);
